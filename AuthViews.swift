    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 18) {
                ...
                Text("Augmented Reality Construction Link")
                        ...
                        HStack(spacing: 14) {
                            VStack(alignment: .leading, spacing: 4) {
                                Text(localized("Open Demo", authLanguage))
                                    .font(.headline)
                                    .foregroundStyle(.primary)
                                Text(localized("Try the seeded Crew Lead and Crew experience without signing in.", authLanguage))
                                    .font(.caption)
                                    .foregroundStyle(.secondary)
        ...
        if createSelectedRole == .worker {
            Section(localized("Crew Access", authLanguage)) {
                ...
                if availableSections.isEmpty {
                    Text(localized("No sections exist yet. Ask your crew lead to create one and share its code word.", authLanguage))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
    ...
    private func createProfileAction() {
        guard canCreateProfile else {
            if createFirstName.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ||
                ...
                validationMessage = "No sections are available yet. A crew lead needs to create one first."
            } else {
                validationMessage = "Enter a valid section code word generated by your crew lead."
            }
